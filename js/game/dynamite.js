...
Dynamite.prototype.goTo = function (row, col) {  // when the dynamite goes to an ally
  let commands = [];
...
Dynamite.prototype.updateValidMoves = function () {
  ...
  for (let piece of this.board.piecesColored[this.color]) {
    ...
    if (piece.constructor.ID === "p") {continue}  // TODO : remove
    if (piece.__proto__.ID === "p") {continue}
    if (! piece.hasMoved()) {
