...
function PiecePrototyped(type, board, row, col) {
  
  this.ID = type.prototype.ID;
  this.MOVES = type.prototype.MOVES;
  this.RANGE = type.prototype.RANGE;
  for (let meth of type.prototype.METH_TO_COPY) {
    this[meth] = type.prototype[meth];
  }
...
PiecePrototyped.preciseTransform = function (piece) {}

// ROOT PROTOTYPE

PiecePrototyped.prototype = {
  ...
  transform: function(pieceClass) {
    ...
    if (this.widget !== null) {
      this.widget.classList.remove(this.color + oldClass.ID);  // TODO : remove
      this.widget.classList.remove(this.color + oldClass.prototype.ID);
      this.widget.classList.add(this.color + this.ID);
